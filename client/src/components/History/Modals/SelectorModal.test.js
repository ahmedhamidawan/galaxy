...
describe("History SelectorModal.vue", () => {
    ...
    async function loadHistoriesInStore() {
        axiosMock
            .onGet(`/api/histories?view=summary&order=update_time&offset=${historyStore.historiesOffset}&limit=10`)
            .reply(200, allHistories.slice(historyStore.historiesOffset, 10));

        axiosMock
            .onGet(`/api/histories/count`)
            .reply(200, 15);
    ...
    it("paginates the histories", async () => {
        ...
        // Get an error for this `TypeError: wrapper.vm.loadMore is not a function`
        // await wrapper.vm.loadMore();
        // Will check for whether the next 5 histories load up next...
